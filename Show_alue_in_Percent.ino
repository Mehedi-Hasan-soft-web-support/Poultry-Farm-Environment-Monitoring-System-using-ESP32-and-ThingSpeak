...
void loop() {
  ...
  // Convert gas sensor values to approximate percentages
  float coPercent = (mq7Value / 1023.0) * 100.0;
  float nh3Percent = (mq135Value / 1023.0) * 100.0;

  Serial.printf("Temp: %.1fC,   Hum: %.1f%%\n", temperature, humidity);
  Serial.printf("CO: %.2f%%, NH3: %.2f%%\n", coPercent, nh3Percent);
  ...
  lcd.setCursor(0, 1);
  lcd.print("CO:");
  lcd.print(coPercent, 1);
  lcd.print("% NH3:");
  lcd.print(nh3Percent, 1);
  lcd.print("%");

  // Upload raw analog values to ThingSpeak
